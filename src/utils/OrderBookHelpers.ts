...
export const groupByTicketSize = (
  ...
  ticketSize: number,
  depth: number,
): { asks: OrderBookPair[]; bids: OrderBookPair[] } => {
  const groupOrders = (
    orders: OrderBookPair[],
    type: OrderSide,
  ): OrderBookPair[] => {
    ...
    const items: OrderBookPair[] = Object.entries(results).map(
      ([price, quantity]): OrderBookPair => [parseFloat(price), quantity],
    );
...
export function fillOrderBooks(array: OrderBookPair[], length = 20) {
  ...
  if (deficit > 0) {
    const lastElement: OrderBookPair = [0, 0];
