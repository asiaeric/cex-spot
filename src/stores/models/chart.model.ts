...
export const chartModel: ChartModel = {
  ...
  getChartHistory: thunk(async (actions, payload, { getState }) => {
    try {
      const toDate = getState().currentTimeQuery;
      // Fetch data in CandlestickData format
      const candlestickData: CandlestickData[] =
        await fetchChartHistory(payload);

      actions.setChartHistory(candlestickData);
