...
const MarketTradeRow = ({ item }: Props) => {
  ...
  const fontColor = item.buyerMaker ? fonts.green : fonts.red500;
  ...
  const decimalLotSize = getDecimalPlaces(currentPair?.baseAsset.decimals || 0);
    ...
    <View
      ...
      <View style={[layout.row]}>
        ...
        <Text style={[fonts.gray50, gutters.marginLeft_40, fontColor]}>
          {item.price
            ? `${formatNumberDisplay(item?.price, decimalTickSize)}`
            : "--"}
        </Text>
      </View>
