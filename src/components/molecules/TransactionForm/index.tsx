...
function TransactionForm({ customStyle }: ITransactionForm) {
  ...
  const { createTransaction } = useStoreActions(
    (actions) => actions.openOrdersModel,
  );
  ...
  const { fee } = useStoreState((state) => state.feeModel);
  const { orderActionError } = useStoreState((store) => store.openOrdersModel);
  ...
  const debouncedValidateInputs = useCallback(() => {
    if (debounceTimer) {
  ...
  const availableAmount = useMemo(() => {
    ...
    return asset?.balance?.availableAmount ?? 0;
      ...
      case "LIMIT":
        return (
          <View style={{ position: "relative" }}>
            <PriceInput
              size={currentPair?.quoteAsset.decimals}
  ...
  return (
    <View style={[gutters.marginRight_12, customStyle]}>
      ...
      <InfoRow
        ...
        value={formatCurrency(parseFloat(finalFee.replace(/\.?0+$/, "")), {
          decimals: 8,
        })}
        affix={
          isBuy ? currentPair?.baseAsset.code : currentPair?.quoteAsset.code
